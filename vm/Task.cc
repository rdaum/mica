...
Task::Task() : generic_vm_entity(), parent_task(0) {
  paged = true;
  task_counter++;
...
Task::Task(Ref<Task> parent, size_t msgid, int pool_id) : generic_vm_entity(), parent_task(parent) {
...
int mica::task_count() { return task_counter; }
...
Var Task::notify(const Var &argument) { return NONE; }

void Task::attachment(const Var &object) {}

void Task::detachment(const Var &object) {}

void Task::terminate() { terminated = true; }

bool Task::is_terminated() { return terminated; }

void Task::resume() {}

Var Task::send(const Var &source, const Var &from, const Var &to, const Var &on,
...
bool Task::activate() {
   ...
   */
  for (vector<Ref<Message> >::iterator ci = children.begin(); ci != children.end(); ci++) {
...
void Task::append_child_pointers(child_set &child_list) {
  for (vector<Ref<Message> >::iterator x = children.begin(); x != children.end(); x++) {
...
mica_string Task::rep() const {
  ...
  char identstr[80];
  snprintf(identstr, 80, "pid: %d tid: %d msg_id %d", pid, tid, msg_id);
...
void Task::serialize_full_to(serialize_buffer &s_form) const {
  ...
  for (vector<Ref<Message> >::const_iterator x = children.begin(); x != children.end(); x++) {
