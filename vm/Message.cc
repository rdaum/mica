...
Message::Message() : generic_vm_entity(), parent_task(0), selector(NONE_SYM) {
  msg_counter++;
  age = ticks = 0;
...
Message::Message(Ref<Task> anc, unsigned int id, unsigned int Iage, unsigned int Iticks, const Var &Isource,
                 const Var &Icaller, const Var &Iself, const Var &on, const Symbol &Iselector,
...
bool Message::operator==(const Message &v2) const {
  return (((Task *)parent_task) == ((Task *)v2.parent_task) && msg_id == v2.msg_id);
}

/** is this a reply?
 */
bool Message::isReply() const { return (isReturn() || isRaise() || isHalt()); }

/** is this a return?
 */
bool Message::isReturn() const { return false; }

/** is this a raise?
 */
bool Message::isRaise() const { return false; }

/** is this a halt?
 */
bool Message::isHalt() const { return false; }

bool Message::isReplyTo(const Ref<Task> &e) const { return ((Task *)parent_task == (Task *)e); }
...
bool Message::isLocal() const { return true; }

void Message::finalize_object() { msg_counter--; }
...
var_vector Message::perform(const Ref<Frame> &parent, const Var &args) {
  ...
  Ref<Task> task(block->make_frame(Ref<Message>(this), slot_result.definer));
...
void Message::serialize_to(serialize_buffer &s_form) const {
  ...
  for (x = args.begin(); x != args.end(); x++) x->serialize_to(s_form);
}

RaiseMessage::RaiseMessage(Ref<Task> parent_task, unsigned int id, unsigned int age,
                           unsigned int ticks, const Var &source, const Var &caller, const Var &to,
                           const Var &on, const Symbol &selector, const Ref<Error> &error,
                           mica_string traceback)
    : Message(parent_task, id, age, ticks, source, caller, to, on, selector, var_vector()),
      trace_str(traceback),
      err(error){};
...
RaiseMessage::RaiseMessage() : Message(), err(Ref<Error>(0)) {}

int mica::msg_count() { return msg_counter; }
