...
vector<ReturnType> micaParser::map_to_vec(
...
micaParser::micaParser(const std::string& in_program) : program(in_program) {}
...
NPtr micaParser::translateStatement(NonterminalStatement* statement) {
  if (statement->sideEffectExpr) {
    ...
    for (vector<NonterminalCatchStatement*>::iterator x = statement->catchList.begin();
         x != statement->catchList.end(); x++) {
...
std::string::iterator find_position(std::string& str, int line, int column) {
...
std::string micaParser::get_range(const pair<int, int>& start, const pair<int, int>& end) {
  return std::string(find_position(program, start.first, start.second),
                     find_position(program, end.first, end.second));
}

NPtr micaParser::translateArgDeclList(NonterminalArgDeclList* args, const NPtr& source,
                                      bool declare) {
  var_vector mandatory(map_to_vec(args->mandatoryVarList.begin(), args->mandatoryVarList.end(),
                                  member_pointer(&micaParser::translateId)));

  var_vector optional(map_to_vec(args->optionalVarList.begin(), args->optionalVarList.end(),
                                 member_pointer(&micaParser::translateId)));
...
static operator_info builtins[] = {"self",     Op::SELF,     "slots",  Op::SLOTS,
                                   "source",   Op::SOURCE,   "caller", Op::CALLER,
                                   "selector", Op::SELECTOR, "args",   Op::ARGS};

static operator_info binary_operations[] = {
    "+",        Op::ADD, "-",        Op::SUB, "/",        Op::DIV,  "*",         Op::MUL, "%",
    Op::MOD,    "=",     Op::EQUAL,  "!=",    Op::NEQUAL, "isA",    Op::ISA,     "&&",    Op::AND,
    "||",       Op::OR,  "|",        Op::BOR, "&",        Op::BAND, "^",         Op::XOR, "<<",
    Op::LSHIFT, ">>",    Op::RSHIFT, "<",     Op::LESST,  ">",      Op::GREATERT};
...
NPtr micaParser::translateExpr(NonterminalExpr* expr) {
  if (expr->parenthesizedExpr) {
    ...
    Var opcode;
    for (int i = 0; i < sizeof(unary_operations) / sizeof(operator_info); i++) {
    ...
    Var opcode;
    for (int i = 0; i < sizeof(builtins) / sizeof(operator_info); i++) {
    ...
    for (map_it = expr->mapEntryList.begin(); map_it != expr->mapEntryList.end(); map_it++) {
    ...
    string str;
    for (iter = expr->stringLiteralList.begin(); iter != expr->stringLiteralList.end(); iter++) {
...
NPtr micaParser::translateBinaryExpr(const NPtr& left, NonterminalBinaryExpr* expr) {
  if (expr->rightArg) {
    Var opcode;
    for (int i = 0; i < sizeof(binary_operations) / sizeof(operator_info); i++) {
...
pair<NPtr, std::string> micaParser::translateFrame(pair<int, int> start_pos,
  ...
  pair<int, int> end_pos(make_pair(frame->endpos->line - 1, frame->endpos->column));
...
pair<NPtr, NPtr> micaParser::translateVerbTemplt(NonterminalVerbTemplt* vs) {
  ...
  for (vector<TerminalId*>::iterator x = vs->moreSels.begin(); x != vs->moreSels.end(); x++) {
  ...
  vector<NPtr> args_vec2(
      map_to_vec(vs->args.begin(), vs->args.end(), member_pointer(&micaParser::translateVerbArg)));
...
pair<NPtr, NPtr> micaParser::translateMsgArgs(NonterminalMsgArgs* vs) {
  ...
  vector<NPtr> args_vec2(
      map_to_vec(vs->args.begin(), vs->args.end(), member_pointer(&micaParser::translateExpr)));
...
NPtr micaParser::translateAssignSlot(NonterminalSlot* slot, NonterminalExpr* expr) {
  /** A slot is one of 4 types:
   *  verbSlot, privateSlot, delegateSlot, nameSlot
   *  Each has a structure: slotName | slotNameExpr
   */
