...
var_vector blockNode::compile(Ref<Block> block, Binding &binding) const {
   ...
   */
  var_vector n_ops;
  for (vector<NPtr>::const_iterator i = statements.begin(); i != statements.end(); i++) {
    NPtr stmt = *i;
...
var_vector qualifiedMessageNode::compile(Ref<Block> block, Binding &binding) const {
...
var_vector scatterAssignNode::compile(Ref<Block> block, Binding &binding) const {
...
var_vector forNode::compile(Ref<Block> block, Binding &binding) const {
  ...
  **/
  ...
  NPtr on_range = new blockNode(nblk(

      /** name = car vname
       */
      new assignNode(name, new unaryNode(new identNode(vname), Var(Op::CAR))),
      // vname = cdr vname;

      new assignNode(vname, new unaryNode(new identNode(vname), Var(Op::CDR))),
      /** branch()
       */
      branch,
  ...
  NPtr select_node = new ifElseNode(new identNode(vname), on_range, on_done);

  NPtr loop_node = new loopNode(new blockNode(nblk(select_node)));

  NPtr forEach = new blockNode(nblk(new varDeclNode(vname, rangeExpr), loop_node));
...
var_vector tryCatchNode::compile(Ref<Block> block, Binding &binding) const {
  ...
  for (vector<Catch>::const_iterator x = catchers.begin(); x != catchers.end(); x++) {
