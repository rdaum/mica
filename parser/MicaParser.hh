...
namespace mica {
  ...
  public:
    ...
    template<typename T> NPtr translateBinaryLhs( T bexpr ) {
    ...
    pair<NPtr, NPtr>
    translateVerbTemplt (Whale::NonterminalVerbTemplt*	      vs );

    pair<NPtr, NPtr>
    translateMsgArgs (Whale::NonterminalMsgArgs*	      vs );
    ...
    NPtr translateAssign   (Whale::NonterminalVar* var,
                	    Whale::NonterminalExpr *expr);
    NPtr translateAssignSlot(Whale::NonterminalSlot* slot,
				    Whale::NonterminalExpr *expr);
    NPtr translateAssignVar(Whale::TerminalId* var,
				    Whale::NonterminalExpr *expr);
    ...
    NPtr translateMessage(Whale::NonterminalMessage* msg,
			  const NPtr &dest );


    NPtr translateArgDeclList( Whale::NonterminalArgDeclList *args,
			       const NPtr & source,
			       bool declare );
  ...
  private:
    std::string get_range( const pair<int, int> &start,
    ...
    vector<ReturnType> 
    map_to_vec( InputIterator begin, InputIterator end,
