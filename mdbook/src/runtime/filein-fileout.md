...
Replacement should be atomic at the unit boundary: a failed filein should not leave half of the
replacement visible. When a filein spans many definitions, the practical implementation may use
smaller internal steps, but the authoring contract should remain "the unit replacement committed" or
"it did not".
...
for `#player`. Fileout recognizes owned policy facts and emits grant blocks instead of long runs of
repeated assertions.
