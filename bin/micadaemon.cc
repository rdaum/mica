...
void daemon_signal_handler(int) { Scheduler::instance->stop(); }
...
class ConnectionTask : public Task {
 ...
 public:
  ConnectionTask(const Var &conn_obj, int socket_fd)
      : Task(0, 0), connection(conn_obj), socket(socket_fd) {
    time_to_live = 0;
    logger.infoStream() << "created network connection task" << log4cpp::eol;
    Scheduler::instance->attach(conn_obj, this);
    disconnected = false;

    try {
      var_vector args;
      Var msg = send(connection, connection, connection, connection, ATTACH_SYM, args);
      msg.perform(this, List::from_vector(args));
    } catch (const Ref<Error> &e) {
      logger.errorStream() << "error during dispatch to :attach (" << e << ")"
                           << log4cpp::eol;
    }
  };
  ...
  void detachment(const Var &object) {
    try {
      var_vector args;
      Var msg = send(connection, connection, connection, connection, DETACH_SYM, args);
      msg.perform(this, List::from_vector(args));
    } catch (const Ref<Error> &e) {
      logger.errorStream() << "error during dispatch to :detach (" << e << ")"
                           << log4cpp::eol;
    }
  ...
  void finalize_object() {
    logger.infoStream() << "destroying network connection task" << log4cpp::eol;
  ...
  void spool() {
    while (!terminated && !disconnected) {
      ...
      r_f = std::find(incoming_buffer.mutable_begin(), incoming_buffer.mutable_end(), '\n');
      ...
      try {
        Var msg = send(connection, connection, connection, connection, RECEIVE_SYM, args);
        msg.perform(this, List::from_vector(args));
      } catch (const Ref<Error> &e) {
        logger.errorStream() << "error during dispatch to :receive (" << e << ")"
                             << log4cpp::eol;
      }
    }
  }
  ...
  void handle_message(const Ref<Message> &reply_message) {
    if (reply_message->isReturn()) {
      ...
      logger.errorStream() << "traceback on connection: " << reply_message->args[1].tostring()
                           << log4cpp::eol;

    } else if (reply_message->isHalt()) {
      logger.errorStream() << "child task of connection halted" << log4cpp::eol;
    }
  }
};
...
void setnonblocking(int sock) {
  ...
  if (opts < 0) {
    logger.critStream() << "error in fcntl(F_GETFL): " << strerror(errno)
                        << log4cpp::eol;
    return;
  }
  opts = (opts | O_NONBLOCK);
  if (fcntl(sock, F_SETFL, opts) < 0) {
    logger.critStream() << "error in fcntl(F_SETFL): " << strerror(errno)
                        << log4cpp::eol;
    return;
  }
  return;
 ...
 */
typedef std::unordered_map<int, Ref<ConnectionTask> > ConnectionMap;
...
void build_select_list() {
  ...
  for (ConnectionMap::iterator c = connections.begin(); c != connections.end(); c++) {
...
void handle_new_connection() {
  ...
  if (connection < 0) {
    logger.critStream() << "error in accepting socket: " << strerror(errno)
                        << log4cpp::eol;
    return;
  }
  setnonblocking(connection);

  Ref<ConnectionTask> connection_task(new ConnectionTask(connection_proto, connection));
...
bool write_to_socket(ConnectionMap::iterator c_it) {
  ...
  int actually_written = write(c_it->first, c_it->second->outgoing_buffer.c_str(), buffer_size);
...
void handle_socks() {
  ...
  for (ConnectionMap::iterator c_it = connections.begin(); c_it != connections.end(); c_it++) {
    bool cleared = false;
  ...
  for (vector<ConnectionMap::iterator>::iterator clear = to_clear.begin(); clear != to_clear.end();
       clear++) {
     ...
     */
    logger.infoStream() << "disconnection.  socket fd: " << c_it->first
                        << log4cpp::eol;
...
void network_loop(short port) {
  ...
  if (listening_socket < 0) {
    logger.critStream() << "error in creating listening socket: " << strerror(errno)
                        << log4cpp::eol;
    return;
  }
  ...
  if (bind(listening_socket, (sockaddr *)&server_address, sizeof(server_address)) < 0) {
    logger.critStream() << "error in bind: " << strerror(errno) << log4cpp::eol;
  ...
  do {
    ...
    readsocks = select(highsock + 1, &read_socks, &write_socks, (fd_set *)0, &timeout);

    if (readsocks < 0) {
      logger.critStream() << "error in select: " << strerror(errno)
                          << log4cpp::eol;
    } else if (readsocks)
      handle_socks();
...
int main(int argc, char *argv[]) {
   ...
   */
  try {
    logger.infoStream() << "starting network listener" << log4cpp::eol;
    network_loop(port);
    logger.infoStream() << "network listen finished" << log4cpp::eol;
  } catch (Ref<Error> e) {
    cout << e << endl;
  }
