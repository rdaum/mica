...
- `src/session.rs`: `MicaSessionStore` which creates, looks up, revokes, and
  updates sessions as Mica relation facts via the task driver.
...
Sessions are stored as Mica relation facts (`source/AuthSession`,
`source/SessionUser`, etc.) so that Mica code running inside the runtime can
inspect and reason about active sessions through ordinary relation queries.
