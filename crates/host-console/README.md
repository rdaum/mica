...
`mica-host-console` is an interactive testing console for the Mica host protocol.

It connects to a daemon ZeroMQ endpoint, opens one host endpoint, and then treats ordinary input
lines like telnet input: the console starts a `read(:line)` task, submits the line as endpoint
input, and then invokes the demo `:command(...)` verb with the selected actor.
