...
It is deliberately small: HTTP/1.1 parsing uses [`httparse`](https://docs.rs/httparse/) and socket
I/O uses `compio` directly. This crate does not use a web framework, Hyper, Axum, or Tokio. HTTP/2,
HTTP/3, TLS, and richer host protocol integration are deferred here; browser WebTransport lives in
...
The default web principal is `#web`; use `mica-daemon --web-principal NAME` to select another
identity with suitable `CanRead` and `CanInvoke` policy facts.

The codec is factored separately from the listener so request parsing can be tested and evolved
without involving sockets.
