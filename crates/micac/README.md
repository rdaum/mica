...
`mica-micac` is the Mica filein compiler. It reads Mica source files, compiles them into a fresh
durable relation store, and optionally runs in check-only mode with no persistent output.

This is a standalone binary crate, not a library. It wraps `mica-runtime` and `mica-relation-kernel`
to provide the `micac` command-line tool.
...
- `--embedding-provider deterministic|disabled|vllm`: embedding provider for compile-time
  operations.

## Role In Mica

`micac` is a development tool for compiling Mica fileins into a fresh database. It is not the
runtime, not a REPL, and not a daemon. It is useful in CI for validating that a set of fileins
compiles cleanly (`--check`) and for pre-building databases from source fileins before deployment.
