...
OStorage *Unserializer::parseOStorage() {
  ...
  while (1) {
    ...
    size_t a_size;
    UnPack(a_size);
    while (a_size--) argument_template.push_back(parseVar());
    Var method(parseVar());

    env->set_verb_parasite(selector, position, argument_template, definer, method);
  }
