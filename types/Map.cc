...
Var Map::add( const Var &v2 ) const
{
  throw unimplemented("map addition");
}

Var Map::sub( const Var &v2 ) const
{
  throw unimplemented("map subtraction");
}

Var Map::mul( const Var &v2 ) const
{
  throw unimplemented("map multiplication");
}

Var Map::div( const Var &v2 ) const
{
  throw unimplemented("map division");
}
...
float Map::tofloat() const {
  throw invalid_type("invalid operand");
}

Var Map::mod( const Var &rhs ) const {
  throw invalid_type("invalid operand");
}
...
Var Map::dropSeq( const Var &n ) const {
  ...
  var_map new_map(*this);
  for (var_vector::iterator fi = flattened.begin(); fi != flattened.end();
       fi++) {
...
bool Map::member( const Var &n ) const {
  return (find(n) != end());
}
...
{
  var_vector ops;
  for (var_map::const_iterator x = this->var_map::begin(); 
       x != this->var_map::end(); x++) {
    ops.push_back( x->first ); ops.push_back( x->second );
  }
  return ops;
}
