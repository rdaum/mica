...
struct ptr_hash {
  ...
  size_t operator()(const T *ptr) const { return (size_t)ptr + 2743 + 5923; }
};
...
struct hash_long_long {
  ...
  inline size_t operator()(unsigned int long long x) const { return x & 0x7fffffff; }
};
...
struct pair_hash {
  ...
  size_t operator()(const std::pair<T, X> &key) const {
...
struct str_hash {
  ...
  size_t operator()(const mica_string &str) const;
};
